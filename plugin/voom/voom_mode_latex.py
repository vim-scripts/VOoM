...
def hook_newHeadline(VO, level, blnum, tlnum):
    ...
    assert lev <= level
    if not lev==level:
        vim.command("call Voom_ErrorMsg('VOoM (LaTeX): MAXIMUM LEVEL EXCEEDED')")
...
def hook_doBodyAfterOop(VO, oop, levDelta, blnum1, tlnum1, blnum2, tlnum2, blnumCut, tlnumCut):
    ...
    if invalid_elems or invalid_sects:
        vim.command("call Voom_ErrorMsg('VOoM (LaTeX): Disallowed levels have been corrected after ''%s''')" %oop)
        if invalid_elems:
            invalid_elems = ', '.join(['%s' %i for i in invalid_elems])
            vim.command("call Voom_ErrorMsg('              level set to 1 for nodes: %s')" %invalid_elems)
        if invalid_sects:
            invalid_sects = ', '.join(['%s' %i for i in invalid_sects])
            vim.command("call Voom_ErrorMsg('              level set to maximum for nodes: %s')" %invalid_sects)
